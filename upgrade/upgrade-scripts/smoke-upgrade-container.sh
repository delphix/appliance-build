...
usage() {
	...
	Runs a quick smoke validation for recent upgrade-container changes:
	  ...
	  3) preview cleanup, then cleanup and verify removal
	EOF
}
...
main() {
	...
	log "Previewing cleanup actions"
	preview_out=$("$UC_SCRIPT" cleanup --preview "$SMOKE_CONTAINER")
	contains_line "$preview_out" "would stop container '$SMOKE_CONTAINER'" ||
		fail "cleanup preview missing stop action"
	contains_line "$preview_out" "would destroy container '$SMOKE_CONTAINER'" ||
		fail "cleanup preview missing destroy action"
	pass "cleanup --preview shows expected actions"
