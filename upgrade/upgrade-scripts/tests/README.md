...
- `cleanup <container>` (without `--force`) reports correct actions and does not mutate state
...
3. Preview and clean up:
	...
	- `../upgrade-container cleanup --force "$c"`
	- `../upgrade-container list-all | grep -Fx "$c" && echo "unexpected" || echo "clean"`

Expected result:

- The container appears in both list commands while running.
- Dry-run prints `would stop` and `would destroy` actions.
- After `cleanup --force`, the container is no longer returned by `list-all`.
