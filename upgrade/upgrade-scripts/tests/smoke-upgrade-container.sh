...
usage() {
	...
	Runs a quick smoke validation for recent upgrade-container changes:
	  ...
	  3) cleanup dry-run, then cleanup --force and verify removal
	EOF
}
...
main() {
	...
	cleanup_on_exit() {
		...
		if [[ -n "$SMOKE_CONTAINER" ]]; then
			"$UC_SCRIPT" cleanup --force "$SMOKE_CONTAINER" >/dev/null 2>&1 || true
		fi
	}
	...
	log "Validating cleanup dry-run actions"
	dryrun_out=$("$UC_SCRIPT" cleanup "$SMOKE_CONTAINER")
	contains_line "$dryrun_out" "would stop container '$SMOKE_CONTAINER' (re-run with --force)" ||
		fail "cleanup dry-run missing stop action"
	contains_line "$dryrun_out" "would destroy container '$SMOKE_CONTAINER' (re-run with --force)" ||
		fail "cleanup dry-run missing destroy action"
	pass "cleanup dry-run shows expected actions"

	log "Running cleanup --force"
	"$UC_SCRIPT" cleanup --force "$SMOKE_CONTAINER"
